...
class JvmJMapManager : JMapManager {
    private val geolocator: Geolocator? = null
    private val geocoder = Geocoder.googleMaps(apiKey = "YOUR API KEY")
    override val isTrackingActive: Flow<Boolean> =
        geolocator?.trackingStatus?.map { it.isActive } ?: flow{emit(false) }
